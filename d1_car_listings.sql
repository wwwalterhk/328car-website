...
CREATE TABLE IF NOT EXISTS model_remarks (
  remark_pk INTEGER PRIMARY KEY AUTOINCREMENT,
  model_pk INTEGER NOT NULL,
  item TEXT NOT NULL,
  remark TEXT NOT NULL,
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  UNIQUE (model_pk, item)
);

CREATE INDEX IF NOT EXISTS idx_model_remarks_model
  ON model_remarks (model_pk);

-- Model options linked to models
CREATE TABLE IF NOT EXISTS model_options (
  ...
  certainty TEXT,
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  UNIQUE (model_pk, item, certainty)
);

CREATE INDEX IF NOT EXISTS idx_model_options_model
  ON model_options (model_pk);
